...
object TxtStreamTest extends LogBase {
  ...
  def main(args: Array[String]): Unit = {
    ...
    log(s"Finished all test of ${this.getClass.getCanonicalName}, test times = ${testTimes}, interval = ${testIntervalSeconds} s. used time = ${(new Date().getTime - beginForAll.getTime) / 1000.0} s, read = ${pathPattern}")
  }
}
