...
echo Current SparkOptions=%SparkOptions% | lzmw -PA -ie "([\w\.]*\.\w*mobius\w*\.[\w\.]*)|SparkOption\w*|=\w+|\s+\d+\w{0,2}(\s+|$)|--jars" -t "((--name))\s+(((\S+)))" -a
echo.
...
if %jarCount% LSS 1 (
    echo Warn : No jars found After-or-No --jars , make sure not need jars or else check again, like MobiusCodeRoot etc. ^( current MobiusCodeRoot=%MobiusCodeRoot% ^) | lzmw -PA -it "--jars|MobiusCodeRoot|(\S+)"
)
